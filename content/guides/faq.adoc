...
When `partial` (or other higher-order function combinators like `comp`, `juxt`, etc) is used, it captures the value of any function vars referenced. For example: `(partial my-fn 100)` captures the current value of `my-fn`. If `my-fn` is rebound in the REPL, the prior `partial` will not "see" those changes, because the var has already been dereferenced to a function value there.

If you are finding this to be a problem in interactive development, you can insert a layer of indirection. One option is to use the var reference `#'myfn` instead or you can use a separate `fn` or `defn` to re-include var dereferencing.

In general, this is not an issue in a running app (because vars are not getting re-bound), but more commonly in interactive REPL development.
