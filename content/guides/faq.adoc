...
pass:[`(.method object fixed-args (into-array type variable-args...))`]
...
If the type is a primitive type, like `double`, then you should instead use the approriate primitive array constructor, like `int-array`, `double-array`, etc. For example:
