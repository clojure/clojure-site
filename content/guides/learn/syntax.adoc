...
[source, clojure]
----
42              ; fixed precision integers - 64-bit integer (from -2^63 to 2^63-1)
-1.5            ; fixed precision floating point - 64-bit float (IEEE 754)
22/7            ; Ratio
42N             ; arbitrary precision integer
1.0M            ; arbitrary precision fixed-point decimal
----

Integers are read as fixed precision when they are in range and arbitrary precision otherwise. A trailing `N` can be used to force arbitrary precision. Clojure also supports the Java syntax for octal (prefix `0`), hexadecimal (prefix `0x`) and arbitrary radix (prefix with base then `r`) integers. Ratios are provided as their own type combining a numerator and denominator.

Floating point values are read as double-precision floats unless the `M` suffix is supplied. Exponential notation is also supported. The special symbolic values `##Inf`, `##-Inf`, and `##NaN` represent positive infinity, negative infinity, and "not a number" values respectively.
