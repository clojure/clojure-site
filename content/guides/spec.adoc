...
http://clojure.github.io/clojure/branch-master/clojure.spec-api.html#clojure.spec/explain[`explain`] is another high-level operation in spec that can be used to report (to `*out*`) why a value does not conform to a spec. Let's see what explain says about some non-conforming examples we've seen so far.
...
Spec also defines one additional regex operator, http://clojure.github.io/clojure/branch-master/clojure.spec-api.html#clojure.spec/&[`&`], which takes a regex operator and constrains it with one or more additional predicates. This can be used to create regular expressions with additional custom constraints that would otherwise require custom predicates. For example, consider wanting to match only sequences with an even number of strings:
...
If the `spec`s were removed this spec would instead match a sequence like `[:names "a" "b" :nums 1 2 3]`.
...
Clojure programs rely heavily on passing around maps of data and this is one of the areas where spec takes a novel approach
compared to other libs. Rather than defining attribute (key+value) specifications at the map level, specs assign meaning to individual attributes,
...
It is also possible to use `fspec` to create an independent function spec that can also be used outside `fdef`:

[source,clojure]
----
(def compare-spec
  (s/fspec
    :args (s/cat :ns (s/+ number?))
    :ret number?))

(s/valid? compare-spec max)
;;=> true
(s/valid? compare-spec min)
;;=> true
----

Here `compare-spec` is a generic spec that specifies a function that takes one or more numbers and returns a number. Both the clojure.core `max` and `min` functions satisfy this function spec. Function specs can also be used to spec higher-order functions.
