...
We mentioned earlier that `clojure.spec.test` provides tools for automatically testing functions. When functions have specs, we can use http://clojure.github.io/clojure/branch-master/clojure.spec-api.html#clojure.spec.test/test[`test`], to automatically generate tests that check the function using the specs. 

`test` will generate arguments based on the `:args` spec for a function, invoke the function, and check that the `:ret` and `:fn` specs were satisfied.
...
[source,clojure]
----
(-> (stest/enumerate-namespace 'user) stest/test)
----

And you can test all of spec'ed functions by calling `stest/test` without any arguments.
