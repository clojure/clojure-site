...
Let's take a moment to examine the explain error output on that final example. There were two different kinds of failures detected. The first failure is due to a missing dispatch method for `:event/restart`. The second is from the invalid `:search/url` value (a number instead of a string) and a missing required `:event/timestamp` key. We see the same parts as prior explain errors:
