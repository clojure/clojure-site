...
brilliant https://yobriefca.se/blog/2014/05/19/the-weird-and-wonderful-characters-of-clojure/[blog post], The weird and wonder characters of Clojure.
====
[[dispatch]]
== `#` - Dispatch character
...
`#` is a reader macro that tells the Clojure reader (the component that takes a
file of Clojure code and "parses" it as Clojure data) to go and
look at another *read table* for the definition of the next character
- in essence this allows extending default reader behaviour.

Clojure doesn't provide support for creating new reader macros.

For `#` used *at the end of a symbol, see <<xref/../weird_characters#gensym,(Gensym)>>
...
are duplicates. Instead the `set` function should be used on a vector.
...
A neat trick is that multple `#_` can be stacked to omit multiple forms
[source,clojure]
----
...
and prevent their evaluation as with the quote function.
[source,clojure]
----
...
they provide faster comparisons and lower memory overhead compared to strings.
...
*template* for the data structure and won't immediately resolve it.
...
`~` is unquote. That is within a syntax quoted (<<xref/../weird_characters#syntax_quote,```>>) form `~` will *unquote*
...
5
----
This forms the meat and potatoes of creating macros which are functions invoked
during compilation that take code and return code.
...
#'user/three-and-four
user=> `(1 ~three-and-four) ; evaluates `three-and-four` and replaces it in the form
(1 (3 4))
user=> `(1 ~@three-and-four) ;  evaluates `three-and-four and replaces with its contents in the form
(1 3 4)
----
...
many LISPs used to denote *special vars*. Most commonly in Clojure this is
...
A `#` *at the end* of a symbol is used to automatically generate a new symbol.
...
Clojure allows `,` to be used as a whitespace character, eg when defining
maps:
[source,clojure]
----
