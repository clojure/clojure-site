...
* Git coordinate: `{:git/url "https://github.com/user/project.git", :rev "tag-name"}`
...
*** http, git protocol - insecure, not recommended
** The `:rev` attribute can be one of the following:
*** A full sha
*** A partial prefix sha, sufficiently unique in the repo
*** An annotated tag name
*** Note: Branch names should not be used!

[source,clojure]
----
{:deps
 {org.clojure/tools.reader {:mvn/version "1.1.1"}
  sally/awesome {:git/url "https://github.com/sally/awesome.git", :rev "awesome-1.2.3"}
  ;; ... add more here
 }}
----
...
The supported git url protocols are https and ssh. https repos will be access anonymously and require no additional authentication information. This approach is recommended for public repos.

ssh repos may be either public or private. Access to a git repo via ssh requires an ssh keypair. The private key of this keypair may or may not have a passphrase. The Clojure tools will connect to and use your ssh agent to validate your ssh key. On *nix, this is typically ssh-agent and on Windows it is Pageant/PuTTY. 

You can verify that your ssh agent is running and has a registered identity using:
...
Which should list at least one identity, typically the key at `~/.ssh/id_rsa`. If no identities are listed, you can add one. This will prompt you for your private key passphrase if your keypair is stored with one:
...
Tip: If you have forgotten your passphrase, and you're using Mac OS X, you may be able to recover it via the keychain. Open Keychain Access, search for `id_rsa`, click "Show password", then enter your OS X user password to retrieve it.
