...
Note that strings must be passed in double quotes, also surrounded by single quotes to escape correctly with bash: `clj -X:my-fn '"foo"'`.
...
* `tool` args (use `-T` or `-A` to name alias)
...
* `resolve-deps` args (use `-R` or `-A` to name alias)
...
* `make-classpath` args (use `-C` or `-A` to name alias)
...
* JVM options (use `-O` or `-A` to name alias)
...
* Main arguments (use `-M` or `-A` to name alias)
