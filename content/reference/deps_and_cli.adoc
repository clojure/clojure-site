...
As of 1.9, Clojure now provides:
...
* Scripts (`clojure` and `clj`) that enable users to make use of this capability at the termainl to declare dependencies, assemble classpaths, and launch Clojure programs
* System-specific installers for downloading the scripts, improving the "Getting Started" experience
...
For example, this deps map specifies two (Maven-based) dependencies:
...
----
{
 org.clojure/clojure           {:mvn/version "1.8.0"
                                :paths [".../clojure-1.8.0.jar"]}
 org.clojure/core.cache        {:mvn/version "0.6.5"
                                :paths [".../core.cache-0.6.5.jar"]} 
 org.clojure/data.priority-map {:mvn/version "0.0.7"
                                :paths [".../data.priority-map-0.0.7"]
                                :dependents [org.clojure/core.cache]}
}
----

The lib map lists all libraries, their selected coordinates, the `:path` on disk, and a list of dependents that caused it to be included. Here you can see that `data.priority-map` was included as a dependency of core.cache.
...
The scripts rely on several directories and optionally on several environment variables.
...
* Config directory
** Holds a deps.edn file that persists across script upgrades and affects all projects
...
The scripts also provide support for connecting to private S3 Maven repositories (thanks to the https://github.com/s3-wagon-private/s3-wagon-private[s3-wagon-private] and https://github.com/spring-projects/aws-maven[aws-wagon] projects).
...
For script installation, see the instructions in the <<xref/../../guides/getting_started#,Getting Started>> guide.
