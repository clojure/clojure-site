...
Additionally, you may list multiple functions to execute in order. The first function will receive the :exec-args and command-line key-path / value map. Subsequent functions will receive the result of the previous function (which will only include those kvs if the first function passes them along).

[source]
----
clj -X my.qualified/fn my.other/fn :config 789
----
...
* `-X:deps help/dir` - prints the public vars in namespace provided with key `:ns` or `:ns-default` if not given
...
The prep task will find all libs in the dependency expansion and look for libs that are source libs, need prep (based on their `:deps/prep-lib` key), and are not yet prepped (based on the `:ensure` dir in their `:deps/prep-lib` map). Those libs will run the comamnd specified there by alias and function.

By default the prep command will use the root and project deps. However, a different basis can be chosen by using the following params:

* `:root` - one of `:standard` (the default root deps install), a string path, or a map
* `:user` - one of `:standard` (usually `~/.clojure/deps.edn`), a string path, or a map
* `:project` - one of `:standard` (`./deps.edn`), a string path, or a map
* `:ext` - a string path or a map
* `:argmaps` - a collection of either alias keywords or literal argmaps to be combined
...
See <<deps_and_cli#find-versions for more on finding versions of a lib or tool (Maven or git).
