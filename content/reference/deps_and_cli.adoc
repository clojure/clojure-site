...
Tools can also be installed and referenced by a simpler local name, instead of via an alias. See <<deps_and_cli#tool_install,tool installation>> for more details.
...
[source,shell]
----
clj -T:deps find-versions ...
----

The args that can be provided are:

* `:git/lib` - git lib name. The git url will be automatically constructed from the git library name. For example, a `:git/lib` of `io.github.clojure/tools.deps.graph` will construct the git url `https://github.com/clojure/tools.deps.graph.git`.
* `:mvn/lib` - maven coordinate (groupId/artifactId), such as `org.clojure/tools.deps.graph`
* `:tool` - a tool name if the tool has already been installed

`find-versions` will print the git or Maven versions, one per line, to the console.

=== Install tool

When you've determined which version of the tool to install, use the `install-tool` program to install the tool by name.

[source,shell]
----
clj -T:deps install-tool ...
---

The args available to `install-tool` are:

* `:mvn/lib` - Maven coordinate (groupId/artifactId)
* `:git/lib` - git lib name (used to construct a git url as per above)
* `:v` - Maven version or git lib sha or tag (will be resolved to a sha)
...
[source,shell]
----
clj -T:deps install-tool :git/lib io.github.clojure/tools.deps.graph :v '"tools.deps.graph-1.0.56"' :as deps-graph
----

=== Tool usage

Tools can publish a `usage.edn` file that declares default configuration. The usage file will be obtained and cached as part of tool installation and used to resolve the function to be executed (and possibly other uses in the future).

The `usage.edn` file is an edn map with the following keys:

* `:exec` - the value is a subset of the alias arguments available in a deps.edn alias:
** `:ns-default`
** `:ns-aliases`

For example, the tools.deps.graph tool might publish the following usage.edn:
...
The usage.edn file is read from the `:paths` of a git lib project or from the root of the maven artifact jar.
...
The function will be resolved using the `:ns-default` and `:ns-aliases` found in the `usage.edn`.
...
* Create a `usage.edn` at the root of one of your `:paths` with either an `:ns-default` and/or an `:ns-aliases` key for your api namespaces
* Tag your git repo to create a release, use a scheme that makes sequencing obvious to users

Instructions for users:

* Find tool versions: `clj -T:deps find-versions :git/lib io.github.USER/TOOL`
* Install tool with `clj -T:deps install-tool :git/lib io.github.ME/TOOL :v '"VERSION"' :as NAME`
