...
[NOTE]
====
This doc is a reference for the latest prerelease version of `clj` (currently 1.10.1.672). Features listed here may differ from those for the current stable release. To install the prerelease, see the instructions on the https://github.com/clojure/homebrew-tools#version-archive-tool-releases[Clojure homebrew tap] or the Linux instructions at the <<xref/../../guides/getting_started#,Getting Started>>.
====
...
{:aliases
 {:install
  {:exec-fn clojure.tools.deps.alpha.tools.install/install
   ;; :exec-args map could be provided but can pass on command line instead
   }}}
----
...
[source,shell]
----
clj -X:deps:install :jar '"/path/to.jar"'
----
...
With the -X exec-opt you can supply key-path/value pairs on the command line. The key-path is either a single key or a vector of keys to refer to a nested key (as with `assoc-in`). Each key-path will be used to `assoc-in` to the original `:args` map, overriding the value there.
