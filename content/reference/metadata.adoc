...
Since Clojure 1.12, the `:param-tags` key is used on method and constructor symbols to specify the arity and parameter types of the desired method. The `:param-tags` vector takes any valid `:tag` value or `_` as a placeholder for non-overloaded parameters.
