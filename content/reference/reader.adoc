...
Metadata can be chained in which case they are merged from right to left. Below example highlight the `:tag java.lang.Integer` type is mapped along with other meta data like `:debug true` & `:private true` by chaining.
