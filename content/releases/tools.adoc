...
** New `-T` option is like `-X` (executes a function) but omits the project `:deps`, replaces project paths with `["."]`. `-T:aliases` is otherwise same as `-X`. `-Ttoolname` resolves named tool by name and uses that tool lib.
** Added new built-in -X:deps <<xref/../../reference/tools#_other_programs,help functions>> `help/doc` and `help/dir`
** Added new API program `prep` that can be used to <<xref/../../reference/deps_and_cli#prep,prep>> source libs
